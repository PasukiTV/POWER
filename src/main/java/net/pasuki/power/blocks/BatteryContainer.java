...
public class BatteryContainer extends AbstractContainerMenu {
    ...
    public BatteryContainer(int windowId, Player player, BlockPos pos) {
        super(Registration.BATTERY_CONTAINER.get(), windowId);
        this.pos = pos;
        if (player.level().getBlockEntity(pos) instanceof BatteryBlockEntity battery) {
            addDataSlot(new DataSlot() {
                ...
                public void set(int pValue) {
                    BatteryContainer.this.power = (BatteryContainer.this.power & 0xffff0000) | (pValue & 0xffff);
                }
            });
            addDataSlot(new DataSlot() {
                @Override
                ...
                @Override
                public void set(int pValue) {
                    BatteryContainer.this.power = (BatteryContainer.this.power & 0xffff) | ((pValue & 0xffff) << 16);
                }
            });
        }
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(player.level(), pos), player, Registration.BATTERY_BLOCK.get());
    }
}
